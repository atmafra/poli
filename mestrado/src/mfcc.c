 ...
 ******************************************************************************/
  ...
  if (file_mode == SINGLE_FILE)
    {
      if (error_if_failure
          (scep_mfcc_file_write (input_file, param, output_file),
           __PROG_NAME_, "error executing feature extraction of file '%s'\n",
           input_file))
        return EXIT_FAILURE;
    }
  else
              ...
              if (output_dir == NULL)
                {
                  if (error_if_null
                      (output_file =
                       (char *) malloc (strlen (input_basename)
                                        + strlen (output_ext) + 1),
                       __PROG_NAME_, strerror (errno)))
                    return EXIT_FAILURE;
