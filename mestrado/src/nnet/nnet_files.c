...
unsigned int cur_attr = 0;      /* current section attribute */
...
nnet_file_read_buffer (const char ignore_char, const char *stdin_prompt)
{
  ...
  if (read_valid_file_line (input_fd, NNET_BUF_SIZE, ignore_char, 0, buf)
      != EXIT_SUCCESS)

    return error_failure ("nnet_file_read_buffer",
                          "error reading valid file line\n");
...
nnet_file_parse_attribute (NFile nfile, CompositeUnion * value)
{
  ...
  if ((strcpy (buf, buf + start_pos) == NULL))
    {
      error_failure ("nnet_file_parse_attribute",
                          ...
                          const char ignore_char, const char *stdin_prompt)
{
  ...
  if (cur_section == -1)
    return error_failure ("nnet_file_read_attribute",
                          "no current section defined\n");
  ...
  while (attribute_found == FALSE && end_section_found == FALSE
         && !feof (input_fd))
    {
      ...
      if (nnet_file_read_buffer (ignore_char, stdin_prompt) != EXIT_SUCCESS)
        return error_failure ("nnet_file_read_attribute",
                              "error reading valid line\n");

      /* Checks if it is a section start tag */
      if (!feof (input_fd))
        {
          /* Checks if the attribute was found */
          while (cur_attr < (unsigned int) nfile[cur_section].nu_attr
          ...
          if (attribute_found == FALSE && end_section_found == FALSE &&
              *buf != '\0')
            error_failure ("nnet_file_read_attribute",
                           "invalid attribute in section %s: '%s'\n",
                           nfile[cur_section].begin, buf);
        }
    }
