                          ...
                          const Vector coord_increment)
{
  ...
  if (som_nnet == NULL)
    return error_failure ("nnet_som_create_nnetwork",
                          "no SOM extension passed\n");
  ...
  if (new_nnet == NULL)
    return error_failure ("nnet_som_create_nnetwork",
                          "error creating new SOM neural network\n");

  /* Creates the input layer */
  input_layer = nnet_layer_create (new_nnet, NULL,
  ...
  output_layer = nnet_layer_create (new_nnet, NULL,
                            ...
                            const TElement element, Unit * winner)
{
  ...
  if (som_nnet == NULL)
    return error_failure ("nnet_som_propagate_element",
  ...
  /*
   * Loads the training element into the input layer
  ...
  if (nnet_nnetwork_activate (som_nnet->nnet, metric) != EXIT_SUCCESS)
    return error_failure ("nnet_som_propagate_element",
                        ...
                        RValue * mse_error)
{
  ...
  RValue elm_mse_error = 0.0;   /* current element MSE error */
  RValue set_mse_error = 0.0;   /* accumulated set MSE error */


  /* checks if the SOM was actually passed */
  if (som_nnet == NULL)
    return null_failure ("nnet_som_propagate_set",
  ...
  winners = vector_create (set->nu_elements);
  ...
  while (cur_element != NULL)
    {
      ...
      if (nnet_som_propagate_element (som_nnet, cur_element, &winner)
          != EXIT_SUCCESS)
        return null_failure ("nnet_som_propagate_set",
      ...
      if (vector_set_value (winners, cur_index, (RValue) winner->unit_index)
          != EXIT_SUCCESS)
        return null_failure ("nnet_som_propagate_set",
                             "error setting winner index for element %ld\n",
                             cur_element->element_index);

      /* calculates this element's MSE error */
      if (calculate_mse_error == TRUE)
        {
          if (nnet_metr_mse_error (cur_element->input, winner, &elm_mse_error)
              != EXIT_SUCCESS)
            return null_failure ("nnet_som_propagate_set",
                                 "error calculating MSE error for element %ld\n",
                                 cur_element->element_index);

          set_mse_error += elm_mse_error;
        }
                        ...
                        RValue * mse_error)
{
  ...
  if (ngb_vector == NULL)
    return error_failure ("nnet_som_train_element",
                          "error creating neighborhood vector\n");
  ...
  /*
   * Loads the training element
   * activates the network
   * finds the winner
   */
  if (nnet_som_propagate_element (som_nnet, element, &winner_unit)
      != EXIT_SUCCESS)
    return error_failure ("nnet_som_train_element",
  ...
  if (nnet_som_ngb_vector (som_nnet, winner_unit, ngb_vector) != EXIT_SUCCESS)

    return error_failure ("nnet_som_train_element",
                          "error getting neighborhood vector\n");
  ...
  while (cur_unit != NULL)
    {
      ...
      if (nnet_unit_get_weight_vector (cur_unit, weight_vector)
          != EXIT_SUCCESS)
        return error_failure ("nnet_som_train_element",
                              "error getting current unit weight vector\n");

      if (vector_subtract (element->input, weight_vector, error_vector)
          != EXIT_SUCCESS)
        return error_failure ("nnet_som_train_element",
                              "error calculating error vector\n");

      if (vector_get_value (ngb_vector, cur_comp, &ngb_value) != EXIT_SUCCESS)
        return error_failure ("nnet_som_train_element",
                              "error getting neighborhood value from vector at component %ld\n",
                              cur_comp);

      rate = ngb_value * learning_rate;

      if (rate > DBL_EPSILON && rate - __RATE_THRESHOLD_ > DBL_EPSILON)
        {
          /*
          ...
          if (vector_scalar_multiply (error_vector, rate, error_vector)
              != EXIT_SUCCESS)
            return error_failure ("nnet_som_train_element",
                                  "error calculating weight correction vector\n");

          if (vector_sum (weight_vector, error_vector, weight_vector)
              != EXIT_SUCCESS)
            return error_failure ("nnet_som_train_element",
                                  "error calculating new weight vector\n");

          if (nnet_unit_set_weight_vector (cur_unit, weight_vector)
              != EXIT_SUCCESS)
            return error_failure ("nnet_som_train_element",
                                  "error updating current unit's weights\n");
        }
  ...
  if (calculate_mse_error == TRUE)
    {
      if (nnet_metr_mse_error (element->input, winner_unit, mse_error)
          != EXIT_SUCCESS)

        return error_failure ("nnet_som_train_element",
                              "error calculating MSE error\n");
    }
  ...
  if (vector_destroy (&ngb_vector) != EXIT_SUCCESS)
    return error_failure ("nnet_som_train_element",
                          "error destroying neighborhood vector\n");

  /* Destroys the auxiliary weight vector */
  if (vector_destroy (&weight_vector) != EXIT_SUCCESS)
    return error_failure ("nnet_som_train_element",
                          "error destroying auxiliary weight vector\n");

  /* Destroys the error vector */
  if (vector_destroy (&error_vector) != EXIT_SUCCESS)
    return error_failure ("nnet_som_train_element",
                          "error destroying error vector\n");
                    ...
                    const BoolValue calculate_mse_error, RValue * mse_error)
{
  ...
  RValue elm_mse_error = 0.0;   /* current element MSE error */
  RValue set_mse_error = 0.0;   /* accumulated set MSE error */
  int rst_flag = 0;             /* flag: reset display */
  ...
  if (nnet_som_ngb_set_parameter (nfunc, 0, (RValue) t) != EXIT_SUCCESS)
    return error_failure ("nnet_som_train_set",
                          "error updating time in neighborhood function\n");

  /* Updates progress bar */
  if (output_progress == TRUE)
    if (display_progress (first_epoch, (long) max_epochs, (long) t,
                          progress_width, progress_character, rst_flag)
        != EXIT_SUCCESS)
      return error_failure ("nnet_som_train_set",
  ...
  while (element != NULL)
    {
      if (nnet_som_train_element (som_nnet, element, etha,
                                  calculate_mse_error, &elm_mse_error)
          != EXIT_SUCCESS)
               ...
               const BoolValue include_out_connections, FILE * output_fd)
{
  ...
  nnet_nnetwork_info (som_nnet->nnet, include_layers_info,
