  ...
  if (extension != NULL)
    {
      switch (extension->index)
        ...
        case NNEXT_SOM:
          if (nnet_som_extend_nnetwork (new_nnet, extension) != EXIT_SUCCESS)
            {
              error_failure ("nnet_create_nnetwork",
                             "error extending neural network to SOM\n");
              return NULL;
            }
          break;

        case NNEXT_LVQ:
          if (nnet_lvq_extend_nnetwork (new_nnet, extension) != EXIT_SUCCESS)
            {
              error_failure ("nnet_create_nnetwork",
                             "error extending neural network to LVQ\n");
              return NULL;
            }
          break;
...
nnet_nnetwork_activate (NNetwork nnet, const VectorMetric vector_metric)
{
  ...
  if (nnet == NULL)
    return error_failure ("nnet_nnetwork_activate",
  ...
  while (cur_layer != NULL)
    {
      ...
      if (nnet_layer_activate (cur_layer, vector_metric) != EXIT_SUCCESS)
        return error_failure ("nnet_nnetwork_activate",
                              "error activating layer %d\n",
