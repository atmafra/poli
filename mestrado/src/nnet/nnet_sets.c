  ...
  while (cur_element != NULL)
    {
      ...
      if (nnet_tset_element_transfer_to_set (cur_element, dest_set,
                                             (ElementIndex *) NULL,
                                             FALSE) != EXIT_SUCCESS)
        return error_failure ("nnet_tset_merge",
                  ...
                  const TSetDivisionCriterion division_criterion)
{
  ...
  if (orig_set == NULL)
    return error_failure ("nnet_tset_divide", "no origin set\n");

  /* Checks the number of elements to transfer */
  if (nu_elements > orig_set->nu_elements)
    return error_failure ("nnet_tset_divide",
                          "invalid number of elements to transfer: %ld\n",
                          nu_elements);

  /* Creates the new training set */
  new_set = nnet_tset_create (new_set_name,
                              orig_set->input_dimension,
                              orig_set->output_dimension);
  if (new_set == NULL)
    return error_failure ("nnet_tset_divide", "error creating new set\n");
  ...
  if (division_criterion == PICK_FROM_BEGGINING ||
      division_criterion == PICK_FROM_END)
    {
      ...
      if (cur_element == NULL)
        return error_failure ("nnet_tset_divide",
                              "error moving to first element to transfer\n");
    }
  else
    {
  ...
  for (elements_done = 0; elements_done < nu_elements; elements_done++)
    {
      ...
      if (nnet_tset_element_transfer_to_set (cur_element, new_set,
                                             NULL, FALSE) != EXIT_SUCCESS)
        return error_failure ("nnet_tset_divide",
                              "error transferring element %ld/%ld to the new set\n",
  ...
  if (set->input_dimension > 0)
    {
      ...
      while (cur_element != NULL)
        {
          if (vcst_add_stat (cur_element->input) != EXIT_SUCCESS)
            {
              vcst_init_stat (0);
              return error_failure ("nnet_tset_update_vector_stats",
                                    "error adding input vector to statistics\n");
            }
          cur_element = cur_element->next;
        }

      /* Gets the resulting statistics vector set */
      if (vcst_stats_update (set->input_vector_stats) != EXIT_SUCCESS)
        return error_failure ("nnet_tset_update_vector_stats",
                              "error getting input statistic vectors set\n");
    }
      ...
      while (cur_element != NULL)
        {
          ...
          if (vcst_add_stat (cur_element->output) != EXIT_SUCCESS)
            {
              vcst_init_stat (0);
              return error_failure ("nnet_tset_update_vector_stats",
                                    "error adding output vector to statistics\n");
            }
          cur_element = cur_element->next;
        }

      /* Gets the resulting statistics vector set */
      if (vcst_stats_update (set->output_vector_stats) != EXIT_SUCCESS)
        return error_failure ("nnet_tset_update_vector_stats",
                              "error getting output statistic vectors set\n");
    }
                      ...
                      const BoolValue regularize_outputs)
{
  ...
  /* Checks if the training set was actually passed */
  if (set == NULL)
    return error_failure ("nnet_tset_regularize", "no training set passed\n");

  /* Checks if the set is not empty */
  if (set->nu_elements == 0)
    return error_failure ("nnet_tset_regularize", "empty set passed\n");
  ...
  if (set->input_vector_stats != NULL)
    {
      ...
      if (input_avg_vector == NULL)
        return error_failure ("nnet_tset_regularize",
                              "undefined input averages\n");

      if (input_pond_vector == NULL)
        return error_failure ("nnet_tset_regularize",
                              "undefined input inverse standard deviation vector\n");
    }
  ...
  if (set->output_vector_stats != NULL)
    {
      ...
      if (output_avg_vector == NULL)
        return error_failure ("nnet_tset_regularize",
                              "undefined output averages\n");

      if (output_pond_vector == NULL)
        return error_failure ("nnet_tset_regularize",
                              "undefined output inverse standard deviations\n");
    }
      ...
      if (regularize_inputs == TRUE && set->input_dimension > 0)
        {
           ...
           */
          if (vector_subtract (cur_element->input, input_avg_vector,
                               cur_element->input) != EXIT_SUCCESS)

            return error_failure ("nnet_tset_regularize",
                                  "error subtracting input average vector\n");

          if (vector_multiply (cur_element->input, input_pond_vector,
                               cur_element->input) != EXIT_SUCCESS)

            return error_failure ("nnet_tset_regularize",
                                  "error ponderating input element\n");
        }
      ...
      if (regularize_outputs == TRUE && set->output_dimension > 0)
        {
           ...
           * Multiplies by the inverse standard deviation
           */
          if (vector_subtract (cur_element->output, output_avg_vector,
                               cur_element->output) != EXIT_SUCCESS)

            return error_failure ("nnet_tset_regularize",
                                  "error subtracting output average vector\n");

          if (vector_multiply (cur_element->output, output_pond_vector,
                               cur_element->output) != EXIT_SUCCESS)

            return error_failure ("nnet_tset_regularize",
                                  "error ponderating output element\n");
        }
...
/*
 * nnet_tset_unregularize
 *
 * Multiplies by standard deviation vector and sums the averages
 */
int
nnet_tset_unregularize (TSet set,
                        const BoolValue unregularize_inputs,
                        const BoolValue unregularize_outputs)
{
  TElement cur_element = NULL;  /* current training set element */
  Vector input_avg_vector = NULL;       /* input average vector */
  Vector output_avg_vector = NULL;      /* output average vector */
  ...
  /* Checks if the training set was actually passed */
  if (set == NULL)
    return error_failure ("nnet_tset_unregularize",
                          "no training set passed\n");

  /* Checks if the set is not empty */
  if (set->nu_elements == 0)
    return error_failure ("nnet_tset_unregularize", "empty set passed\n");

  /* Trivial case: nothing to do */
  if (unregularize_inputs == FALSE && unregularize_outputs == FALSE)
    return EXIT_SUCCESS;
  ...
  if (set->input_vector_stats != NULL)
    {
      input_avg_vector = set->input_vector_stats->average;
      input_pond_vector = set->input_vector_stats->stddev;
      ...
      if (input_pond_vector == NULL)
        return error_failure ("nnet_tset_unregularize",
                              "undefined input standard deviation vector\n");
    }

  if (set->output_vector_stats != NULL)
    {
      output_avg_vector = set->output_vector_stats->average;
      output_pond_vector = set->output_vector_stats->stddev;
      ...
      if (output_pond_vector == NULL)
        return error_failure ("nnet_tset_unregularize",
                              "undefined output standard deviations\n");
    }

  /* Checks if the statistic vectors are valid */
  if (unregularize_inputs == TRUE && set->input_dimension > 0 &&
      (input_avg_vector == NULL || input_pond_vector == NULL))

    return error_failure ("nnet_tset_unregularize",
                          "input statistics are out of date\n");

  if (unregularize_outputs == TRUE && set->output_dimension > 0 &&
      (output_avg_vector == NULL || output_pond_vector == NULL))
  ...
  while (cur_element != NULL)
    {
      ...
      if (unregularize_inputs == TRUE && set->input_dimension > 0)
        {
          /*
           * Multiplies by the standard deviation
           * Sums the average
           */
          if (vector_multiply (cur_element->input, input_pond_vector,
                               cur_element->input) != EXIT_SUCCESS)
          ...
          if (vector_sum (cur_element->input, input_avg_vector,
                          cur_element->input) != EXIT_SUCCESS)

            return error_failure ("nnet_tset_unregularize",
                                  "error adding input average vector\n");
      ...
      if (unregularize_outputs == TRUE && set->output_dimension > 0)
        {
          ...
          if (vector_sum (cur_element->output, output_avg_vector,
                          cur_element->output) != EXIT_SUCCESS)
...
nnet_tset_read_from_file (TSet set,
                          ...
                          const BoolValue regularize_outputs)
{
  FILE *fp = NULL;              /* pointer to file descriptor */
  char buf[FILE_LINE_SIZE];     /* read buffer */
  char *word = NULL;            /* auxiliar character */
  const char delim[] = " ";     /* number delimiter in a line */
  ...
  UnitIndex new_pos = 1;        /* new value position */
  ElementIndex new_index __attribute__ ((unused)) = 0;  /* new element index */
  RValue new_value = 0.0;       /* new value */
  UnitIndex dim_cnt = 0;        /* vector dimension counter */
  UnitIndex inpdim_cnt = 0;     /* input vector dimension counter */
  UnitIndex outdim_cnt = 0;     /* output vector dimension counter */

  /* NULL checks */
  if (set == NULL)
    return error_failure ("nnet_tset_read_from_file",
                          "no training set passed \n");

  if (file_name == NULL)
    return error_failure ("nnet_tset_read_from_file", "no file name\n");
  ...
  if (update_vector_stats == FALSE &&
    ...
    return error_failure ("nnet_tset_read_from_file",
                          "unable to regularize set without updating statistics\n");
  ...
  while (!feof (fp))
    {
      /* Reads one line */
      if (read_valid_file_line (fp, NNET_BUF_SIZE, IGNORE_TOKEN, 1, buf)
          != EXIT_SUCCESS)
      ...
      if (!feof (fp))
        {
          ...
          if ((word = strtok (NULL, delim)) != NULL)
            ...
            return error_failure ("nnet_tset_read_from_file",
                                  "error reading component value: '%s'\n",
                                  buf);

          dim_cnt++;

          if (dim_cnt <= set->input_dimension)
            {
              inpdim_cnt++;
            }
          else if (dim_cnt <= set->input_dimension + set->output_dimension)
            {
              outdim_cnt++;
            }
          else
          ...
          if (inpdim_cnt == 1 && set->input_dimension > 0)
            {
          ...
          if (outdim_cnt == 1 && set->output_dimension > 0)
            {
              ...
              if (new_output == NULL)
                {
                  fclose (fp);
                  return error_failure ("nnet_tset_read_from_file",
                                        "error creating output vector\n");
                }
              aux_vector = new_output;
            }

          /* Sets the value of the dimension */
          if (vector_set_value (aux_vector, new_pos, new_value)
              != EXIT_SUCCESS)
            {
              fclose (fp);
              return error_failure ("nnet_tset_read_from_file",
          ...
          if ((inpdim_cnt == set->input_dimension ||
               set->input_dimension == 0) &&
              (outdim_cnt == set->output_dimension ||
               set->output_dimension == 0))
            {
              /* Creates a new element with the old input and output  */
              new_element =
                nnet_tset_element_create (set, NULL, new_input, new_output,
              ...
              if (new_element == NULL)
                {
                  fclose (fp);
                  return error_failure ("nnet_tset_read_from_file",
                                        "error creating new training element\n");
                }
            }
        }
    }
  ...
  if (update_vector_stats == TRUE)
    {
       ...
       */
      if (nnet_tset_update_vector_stats (set) != EXIT_SUCCESS)
        return error_failure ("nnet_tset_read_from_file",
                              "error updating statistics\n");

      if (regularize_inputs == TRUE || regularize_outputs == TRUE)
        {
          if (nnet_tset_regularize (set, regularize_inputs,
                                    regularize_outputs) != EXIT_SUCCESS)
            return error_failure ("nnet_tset_read_from_file",
                                  "error ponderating training set\n");

          if (nnet_tset_update_vector_stats (set) != EXIT_SUCCESS)
            return error_failure ("nnet_tset_read_from_file",
                                  "error updating statistics ponderation\n");
        }
    }
                            ...
                            const char *file_name)
{
  ...
  if (new_set == NULL)
    return null_failure ("nnet_tset_create_from_file",
                         "error creating new training set\n");

  /* Reads its contents from the given file */
  if (nnet_tset_read_from_file (new_set, file_name,
                                normalize_input, normalize_output,
                                update_vector_stats,
                                regularize_inputs, regularize_outputs)
      != EXIT_SUCCESS)
    {
      error_failure ("nnet_tset_create_from_file",
                     "error reading training set file\n");

      if (nnet_tset_destroy (&new_set, TRUE) != EXIT_SUCCESS)

        error_failure ("nnet_tset_create_from_file",
                       "error destroying set\n");
                            ...
                            const char *list_file_name)
{
  ...
  if (list_file_name == NULL)
    return null_failure ("nnet_tset_create_from_list",
                         "no input list file name passed\n");
  ...
  while (!feof (inlist_fd))
    {
      ...
      if (!feof (inlist_fd))
        if (nnet_tset_read_from_file (aux_set, set_file, normalize_input,
  ...
  if (nnet_tset_destroy (&aux_set, TRUE) != EXIT_SUCCESS)
...
/*
 * nnet_tset_create_from_table
 *
 * Creates a new training set and reads its contents
 * from a table whose first column is the name of the input file
 */
TSet
nnet_tset_create_from_table (const Table file_table,
                             const UnitIndex input_dimension,
                             const UnitIndex output_dimension,
                             const BoolValue normalize_input,
                             const BoolValue normalize_output,
                             ...
                             const BoolValue regularize_outputs)
{
  TSet t_set = NULL;            /* new training set */
  TSet aux_set = NULL;          /* auxiliary input training set */
  Row cur_row = NULL;           /* current table row */
  BoolValue last_row = FALSE;   /* last row fetched? */
  CompositeUnion cellval;       /* table cell with file name */
  BoolValue nullval;            /* null attribute of cell */
  ...
  /* checks if the file table was passed */
  if (file_table == NULL)
    return null_failure ("nnet_tset_create_from_table",
                         "no file table passed\n");

  if (file_table->rows == 0)
    return null_failure ("nnet_tset_create_from_table", "empty file table\n");

  if (file_table->columns == 0)
    return null_failure ("nnet_tset_create_from_table",
                         "file table has no columns\n");

  if (file_table->first_column->type != STRING)
    return null_failure ("nnet_tset_create_from_table",
                         "first column must be a string\n");

  /* initialization */
  t_set = nnet_tset_create (file_table->name,
  ...
  split_file_name (file_table->ctlfile, &ctl_dir, &ctl_base, &ctl_ext);

  /* processes all files in table */
  while (last_row == FALSE)
    {
      table_fetch (file_table, &last_row, &cur_row);

      if (cur_row == NULL)
        return null_failure ("nnet_tset_create_from_table",
                             "empty row in file table\n");

      if (table_get_cell (cur_row, file_table->first_column,
      ...
      set_file = (char *) malloc (strlen (cellval.stringvalue) +
                                  strlen (ctl_dir) + 1);

      if (set_file == NULL)
        return null_failure ("nnet_tset_create_from_table",
                             "can't get memory for set file name\n");

      sprintf (set_file, "%s%s", ctl_dir, cellval.stringvalue);

      /* creates the auxiliary training set with the current file */
      if (nnet_tset_read_from_file (aux_set, set_file,
                                    normalize_input, normalize_output,
                                    update_vector_stats,
                                    regularize_inputs,
                                    regularize_outputs) != EXIT_SUCCESS)
      ...
      /* merges the auxiliary set with the real training set */
      if (nnet_tset_merge (aux_set, t_set) != EXIT_SUCCESS)
        return null_failure ("nnet_tset_create_from_table",
                             "error merging training set with file '%s'\n",
                             set_file);
  ...
  if (update_vector_stats == TRUE)
    if (nnet_tset_update_vector_stats (t_set) != EXIT_SUCCESS)
      return null_failure ("nnet_tset_read_from_table",
                           "error updating statistics\n");

  /*
     if (regularize_inputs == TRUE || regularize_outputs == TRUE)
     {
     if (nnet_tset_regularize (t_set, regularize_inputs,
     regularize_outputs) != EXIT_SUCCESS)
