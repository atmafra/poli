 ...
 */
int
vector_str_to_metric (const char *str, VectorMetric * metric)
{
  if (str == NULL)
    {
      fprintf (stderr, "vector_str_to_metric: no string passed\n");
      return EXIT_FAILURE;
    }

  if (strcmp (str, __EUCLIDEAN_STRING_) == 0)
    {
      *metric = VECTOR_METR_EUCLIDEAN;
      return EXIT_SUCCESS;
    }

  if (strcmp (str, __INNER_PRODUCT_STRING_) == 0)
    {
      *metric = VECTOR_METR_INNER_PRODUCT;
      return EXIT_SUCCESS;
    }

  fprintf (stderr, "vector_str_to_metric: unknown vector metrics: '%s'\n",
           str);
  return EXIT_FAILURE;
}
 ...
 */
char *
vector_metric_to_str (const VectorMetric metric)
{
  switch (metric)
    {
    case VECTOR_METR_EUCLIDEAN:
      return __EUCLIDEAN_STRING_;

    case VECTOR_METR_INNER_PRODUCT:
      return __INNER_PRODUCT_STRING_;

    default:
      fprintf (stderr, "vector_metric_to_str: unknown vector metrics\n");
      return NULL;
    }
}
 ...
 */
int
vector_reset (Vector vector, const RValue value)
{
  UsLgIntValue cur_comp;        /* auxiliary current component */

  if (vector == NULL)
    return error_failure ("vector_reset", "no vector passed");

  for (cur_comp = 1; cur_comp <= vector->dimension; cur_comp++)

    if (vector_set_value (vector, cur_comp, value) != EXIT_SUCCESS)

      return error_failure ("vector_reset",
                            "error setting value of component %ld\n",
                            cur_comp);
                  ...
                  FILE * output_fd)
{
  ...
  for (cur_dim = 1; cur_dim <= vector->dimension; cur_dim++)
    {
      if (vector_get_value (vector, cur_dim, &cur_val) != EXIT_SUCCESS)
        {
          error_failure ("vector_list_info",
                         "error getting component %ld from vector\n",
                         cur_dim);
          return;
        }

      fprintf (output_fd, "%f %f %f\n", element_index, cur_dim - 1, cur_val);
    }
}
