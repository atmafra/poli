                        ...
                        const RValue value, const MatrixOperation operation)
{
  ...
  if (matrix == NULL)
    {
      fprintf (stderr, "matrix_value_operation: no matrix passed\n");
      return EXIT_FAILURE;
    }
  ...
  if (row < 1 || row > matrix->rows)
    {
      fprintf (stderr, "matrix_value_operation: invalid row index (%ld)\n",
               row);
      return EXIT_FAILURE;
    }
  ...
  if (column < 1 || column > matrix->columns)
    {
      fprintf (stderr, "matrix_value_operation: invalid column index (%ld)\n",
               column);
      return EXIT_FAILURE;
    }
  ...
  switch (operation)
    ...
    default:
      fprintf (stderr,
               "matrix_value_operation: invalid operation requested\n");
      return EXIT_FAILURE;
    }
  ...
  for (row = 1; row <= matrix->rows; row++)
    {
      for (col = 1; col <= matrix->columns; col++)
        {
          exit_status = matrix_get_value (matrix, row, col, &cur_val);
          if (exit_status != EXIT_SUCCESS)
            {
              fprintf (stderr,
                       "matrix_raw_info: error getting element (%ld,%ld) from matrix\n",
                       row, col);
              return;
            }
