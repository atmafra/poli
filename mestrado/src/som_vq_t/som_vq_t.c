...
usage (void)
{
  ...
  puts ("              [-i  | --input <file>]");
  puts ("              [-it | --input-table <control file>]");
  ...
  puts ("  -it | --input-table     table containing list of input files");
...
main (int argc, char **argv)
{
  ...
  char *inlist_file = NULL;     /* files table control file */
  ...
  Table file_table = NULL;      /* input files table */
  ...
  RValue mse_error = 0.0;       /* training set MSE error */
  ...
  InputParameterSet pset = {
    ...
    {"-i", "--input", STRING, FALSE, FALSE,
     {.stringvalue = (char *) NULL}},
    {"-it", "--input-table", STRING, FALSE, FALSE,
     {.stringvalue = (char *) NULL}},
  ...
  InputParameterList plist = { 15, pset };
 ...
 ******************************************************************************/
  ...
  if (inlist_file != NULL)
    {
      file_table = table_create_from_control_file (inlist_file);
      if (file_table == NULL)
        return error_failure (__PROG_NAME_,
                              "error creating table of input files\n");

      if (file_table->columns < 2)
        return error_failure (__PROG_NAME_,
      ...
      if (file_table->first_column->next_column->type != UNSIGNED_LONG_INT)
        return error_failure (__PROG_NAME_,
                              "second column in file table must be desired class, UNSIGNED_LONG_INT\n");

      if (table_read_data (file_table) != EXIT_SUCCESS)
        return error_failure (__PROG_NAME_,
  ...
  if (file_mode == SINGLE_FILE)
    {
      ...
      t_set = nnet_tset_create_from_file ("SOM Training Set",
                                          input_dim, 0,
  ...
  else
    {
      ...
      t_set = nnet_tset_create_from_table (file_table, input_dim, 0,
                                           FALSE, FALSE, TRUE, TRUE, FALSE);
      if (t_set == NULL)
        {
          puts ("FAILED");
  ...
  if (trn_flag == TRUE && rnd_flag == TRUE)
    {
      ...
      if (nnet_tset_randomize (t_set) != EXIT_SUCCESS)
        {
          puts ("FAILED");
          return error_failure (__PROG_NAME_,
      ...
      for (epoch = first_epoch; epoch <= max_epochs; epoch++)
        {
          ...
          if (nnet_som_train_set (som_nnet, t_set,
                                  first_epoch, max_epochs,
                                  rst_flag, TRUE,
                                  __PROGRESS_BAR_WIDTH_,
                                  __PROGRESS_BAR_CHAR_,
                                  mse_flag, &mse_error) != EXIT_SUCCESS)
          ...
          if (mse_flag == TRUE)
            {
              ...
              fprintf (mse_fd, "%f\n",
                       mse_error / (RValue) t_set->nu_elements);
          ...
          if (tm_flag == TRUE)
            {
              ...
              trmap =
                trmap_create_from_vector (output_dim, winners, FALSE, TRUE);
              if (trmap == NULL)
                return error_failure (__PROG_NAME_,
              ...
              matrix_vector_info (trmap, 0.0, tm_fd);
              fclose (tm_fd);

              /* Destroys state transition map */
              if (trmap_destroy (&trmap) != EXIT_SUCCESS)
                return error_failure (__PROG_NAME_,
          ...
          while (last_row == FALSE)
            {
              ...
              if (table_get_cell (cur_row,
                                  file_table->first_column->next_column,
                                  &cellval, &nullval) != EXIT_SUCCESS)
                return error_failure (__PROG_NAME_,
              ...
              if (tm_flag == TRUE)
                {
                  ...
                  trmap = trmap_create_from_vector (output_dim, winners,
                                                    FALSE, TRUE);
                  if (trmap == NULL)
                    return error_failure (__PROG_NAME_,
                                          "error creating state transition map\n");

                  /* writes state transition map and desired class to file */
                  tm_fd = fopen (tm_file, "w");
                  matrix_vector_info (trmap, 0.0, tm_fd);
 ...
 ******************************************************************************/
  ...
  if (nnet_tset_destroy (&t_set, TRUE) != EXIT_SUCCESS)
    {
      puts ("FAILED");
  ...
  if (file_table != NULL)
    {
      ...
      if (table_drop (&file_table) != EXIT_SUCCESS)
        {
          puts ("FAILED");
