...
inpr_param_info (const InputParameter p, FILE * output_fd)
{
  ...
  switch (p.datatype)
    ...
    default:
      fprintf (stdout, "inpr_param_info: invalid parameter type\n");
      break;
    }
  ...
  for (cur_param = 0; cur_param < plist.nu_parameters; cur_param++)
    {
      if (plist.parameter[cur_param].mandatory == TRUE &&
        ...
        {
          fprintf (stderr,
                   "inpr_check_mandatory: mandatory parameter '%s' ('%s') missing\n",
...
inpr_parse (int argc, char **argv, InputParameterList plist)
{
  ...
  UsIntValue val_param;         /* parameter waiting value */
      ...
      if (value_flag == FALSE)
        {
              ...
              if (strcmp (plist.parameter[cp].tag, argv[cur_arg]) == 0 ||
                  strcmp (plist.parameter[cp].tag_syn, argv[cur_arg]) == 0)
                {
                  ...
                  switch (plist.parameter[cp].datatype)
                    ...
                    default:
                      fprintf (stdout,
                               "inpr_parse: invalid parameter type\n");
                      return EXIT_FAILURE;
                    }
                }
              cp++;
          ...
          if (valid_flag == FALSE)
            {
              fprintf (stderr, "inpr_parse: invalid parameter: %s\n",
                       argv[cur_arg]);
              return EXIT_FAILURE;
            }
        }
  ...
  /* Checks if any mandatory parameter is missing */
  exit_status = inpr_check_mandatory (plist);
  if (exit_status != EXIT_SUCCESS)
    {
      fprintf (stderr, "inpr_parse: missing mandatory parameter\n");
      return EXIT_FAILURE;
    }
