 ...
 * strip_nl
 *
 * Strips off the first new line character the string
 */
void
                 ...
                 char **basename, char **extension)
{
  ...
  int dot_pos = -1;             /* dot position */
  int bar_pos = -1;             /* bar position */
  ...
  for (pos = 0; pos <= (int) len; pos++)
    {
      ...
      /* base name part */
      if ((pos > bar_pos) &&
          ((pos < dot_pos && dot_fl) || (pos < (int) len && !dot_fl)) &&
          base_fl)
        {
      ...
      /* end of base name */
      if ((pos == dot_pos || (pos == (int) len && !dot_fl)) && base_fl)
        {
          (*basename)[basename_size - 1] = '\0';
        }

      /* extension part */
      if (pos >= dot_pos && pos < (int) len && dot_fl)
        (*extension)[pos - dot_pos] = file_name[pos];

      /* end of extension */
      if (pos == (int) len && dot_fl)
...
read_file_line (FILE * input_fd, const size_t max_size, char *line)
{
  ...
  /* Reads the file */
  line_read = fgets (line, max_size + 1, input_fd);
                  ...
                  const size_t width, const char symbol, const int reinit)
{
  ...
  for (; nu_symbols_old < nu_symbols_new; ++nu_symbols_old)
    {
      if (nu_symbols_old == 0 || nu_symbols_old == width - 1)
        printf ("%c", '|');
      else
        printf ("%c", symbol);
    }
