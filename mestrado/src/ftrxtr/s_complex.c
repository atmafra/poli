...
{
  va_list inlist;               /* List of input arguments */
  ...
  if ((*function) (retval, mode, inpc, &inlist, parc, parlist) !=
      EXIT_SUCCESS)

    return error_failure ("cmp_fcn_exec",
...
{
  cmp_complex aux_z;            /* Auxiliary input value */

  /* This function accepts only one input argument */
  if (inpc != 1)
    return error_failure ("cmp_mod",
                          "only one argument is accepted for this function\n");

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_mod",
                          "only complex mode is allowed for function modulus\n");
         ...
         const size_t parc, va_list * parlist)
{
  ...
  /* This function accepts only one input argument */
  if (inpc != 1)
    return error_failure ("cmp_sqr",
                          "only one argument is accepted for this function\n");

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_sqr",
                          "only complex mode is allowed for function square\n");
         ...
         const size_t parc, va_list * parlist)
{
  ...
  /* This function accepts only one input argument */
  if (inpc != 1)
    return error_failure ("cmp_arg",
                          "only one argument is accepted for this function\n");

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_arg",
                          "only complex mode is allowed for function argument\n");
         ...
         size_t inpc, va_list * inlist, const size_t parc, va_list * parlist)
{
  ...
  /* This function accepts only one input argument */
  if (inpc != 1)
    return error_failure ("cmp_minus",
                          "only one argument is accepted for this function\n");
         ...
         const size_t parc, va_list * parlist)
{
  ...
  /* This function accepts only one input argument */
  if (inpc != 1)
    return error_failure ("cmp_cnj",
                          "only one argument is accepted for this function\n");

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_cnj",
                          "only complex mode is allowed for function conjugate\n");
...
cmp_sum (cmp_complex * retval, const cmp_fcn_mode_type mode,
         ...
         const size_t parc, va_list * parlist)
{
  size_t aux_arg;               /* Auxiliary argument counter */
  cmp_complex aux_z;            /* Auxiliary argument value */
...
{
  size_t aux_arg;               /* Auxiliary argument counter */
  cmp_real k;                   /* Auxiliary real constant */
  cmp_complex aux_z;            /* Auxiliary complex */

  /* This function accepts only one input argument */
  if (inpc != 1)
    return error_failure ("cmp_rpd",
                          "only one argument is accepted for this function\n");
...
{
  size_t aux_arg;               /* Auxiliary argument counter */
  cmp_complex aux_z, aux_z2;    /* Auxiliary complexes */

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_tms",
                          "only complex mode is allowed for function complex times\n");
...
cmp_inv (cmp_complex * retval, const cmp_fcn_mode_type mode,
         ...
         const size_t parc, va_list * parlist)
{
  cmp_real den;                 /* Auxiliary denominator value */
  cmp_complex aux_z;            /* Auxiliary complex */

  /* Validates the number of input arguments */
  if (inpc != 1)
    return error_failure ("cmp_inv",
                          "this function accepts only one input argument\n");

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_inv",
                          "only complex mode is allowed for function complex invert\n");
  ...
  if (den < DBL_EPSILON && den > -DBL_EPSILON)
    return error_failure ("cmp_inv", "non-invertible complex\n");
...
{
  cmp_complex aux_z;            /* Auxiliary complex */
  cmp_real aux_exp;             /* Auxiliary real */

  /* Validates the number of arguments */
  if (inpc != 1)
    return error_failure ("cmp_exp", "function requires only one argument\n");

  /* Only complex mode is accepted */
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_exp",
                          "only complex mode is allowed for function complex invert\n");
...
cmp_rlg (cmp_complex * retval, const cmp_fcn_mode_type mode,
         ...
         const size_t parc, va_list * parlist)
{
  cmp_complex aux_z;            /* Auxiliary complex number */
  cmp_real basis;               /* Logarithm basis */

  /* Validates the number of input arguments */
  if (inpc != 1)
    return error_failure ("cmp_rlg",
                          "this function requires exactly one input\n");

  /* Validates the number of parameters */
  if (parc != 1)
    return error_failure ("cmp_rlg",
                          "this function requires exactly one parameter\n");
  ...
  if (aux_z.re < DBL_EPSILON || aux_z.im < DBL_EPSILON)
    return error_failure ("cmp_rlg", "invalid rlg argument: %f + j %f\n",
                          aux_z.re, aux_z.im);
  ...
  if (basis < DBL_EPSILON ||
    ...
    return error_failure ("cmp_rlg", "invalid log basis: %f\n", basis);
         ...
         const size_t parc, va_list * parlist)
{
  ...
  if (mode != CMP_FCN_COMPLEX)
    return error_failure ("cmp_rlm",
                          "only complex mode is allowed for function log of modulus\n");
  ...
  if (basis < DBL_EPSILON ||
      (basis < 1.0 + DBL_EPSILON && basis > 1.0 - DBL_EPSILON))
    return error_failure ("cmp_rlm", "invalid log basis: %f\n", basis);
